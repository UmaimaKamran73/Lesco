...
public class Customer implements Serializable
{
    ...
    public String custID; // unique 4 digit(search for limit of digits thingy
    public String CNIC; // same search for 3 numbers without dashes
    public String custName; // comma check for every function almost. only letters and spaces
    public String custAddress; // checks
    public String phoneNumber; // checks and give a format to user too
    public CustomerType custType; // 2 options(commercial or domestic) so search for that. 1,0 input or the
                                  // enumeration thingy (dbms)
    public MeterType meterType; // same thing 2 options(Single Phase or Three Phase)
    ...
    // AGREGATION
    public ArrayList<Billing> billingHistory;

    private static final DateTimeFormatter DATE_FORMATTER = DateTimeFormatter.ofPattern("dd/MM/yyy");

    public Customer(String custID, String CNIC, String custName, String custAddress, String phoneNumber,
            CustomerType custType, MeterType meterType, LocalDate connectionDate, int regularUnitsConsumed,
            int peakHourUnitsConsumed) {
        this.custID = custID;
        this.CNIC = CNIC;
