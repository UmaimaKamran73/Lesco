        ...
        try
        {
            ...
            while((msg=in.readObject())!=null)
            {
                if(msg instanceof String)
                {
                    ...
                    else if(request.startsWith("getCNIC"))
                    {
                        String cnic=customerController.getCNIC();
                        out.writeObject(cnic);
                    }
                    else if(request.startsWith("getExpiryDate"))
                    {
                        LocalDate date=customerController.getExpiryDate();
                        out.writeObject(date);
                    }
                    else if(request.startsWith("updateCNICExpiryDate")) //Assume updateCNICExpiryDate,newExpiryDate
                    {
                        boolean updated=customerController.updateCNICExpiryDate(request);
                   ...
                   /* else if(request.startsWith("updateTarrif"))
                    {
                        String[] params = request.split(",", 2);
                        TarrifTax tarrif = params[1];  //to do somethingabout it
                   ...
                   /* else if(request.startsWith("isMostRecentOrUnpaid"))
                    {
                        String[] params = request.split(",", 2);
                        Billing bill = params[1]; // Populate with correct values from params
                   ...
                   /* else if(request.startsWith("editCustomer"))
                    {
                        String[] params = request.split(",", 2);
                        Customer customer = params[1]; // Populate with correct values from params
